"""Particle coalesce effect for the SWARN ML ENGINEER logo.

Random particles swirl in from the edges, converge to form the text
"SWARN ML INTERN", hold briefly, then the final frame is printed.
...
from agent.utils.classic.braille import BrailleCanvas, text_to_pixels
from agent.utils.classic.boot_timing import settle_curve, warm_gold_from_white
...
def run_particle_logo(
    ...
    converge_seconds: float = 0.9,
    line1: str = "SWARN",
    line2: str = "ML ENGINEER",
) -> None:
    """Run the particle coalesce effect.

    Args:
        console: Rich console to render into.
        hold_seconds: how long the settled logo stays on screen.
        converge_seconds: how long the particles take to fly in and form the
            text. Total runtime is converge_seconds + hold_seconds.
        line1, line2: the two lines of text the particles converge into.
            Uppercase A–Z, 0–9 and space only (see braille.py's bitmap font);
    ...
    text_pixels_line1 = text_to_pixels(line1, scale=2)
    text_pixels_line2 = text_to_pixels(line2, scale=2)
    ...
    fps = 24
    converge_frames = max(1, int(fps * converge_seconds))
    ...
    for line in canvas.render():
        for ch in line:
            if ch == chr(0x2800):
                ...
                # final.append(ch, style="rgb(182,80,255)")
