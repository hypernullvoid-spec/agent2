...
_PLACEHOLDERS = [str(x) for x in T.prompt_placeholders()] or ["ask the Wired"]
...
def read_user_input(prompt: str | None = None) -> str:
    ...
    (see the classic theme's version); styled with this theme's prompt
    symbol and colors. Falls back to Rich input without prompt_toolkit."""
    prompt = prompt if prompt is not None else T.branding("prompt_symbol", "> ")
...
_THINK_FRAMES   = [str(f) for f in T.spinner("thinking_faces")] or ["(◈)"]
_THINK_VERBS    = [str(v) for v in T.spinner("thinking_verbs")] or ["thinking"]
_THINK_LABEL    = T.spinner_opt("thinking_label", "thinking")
_THINK_COLOR    = T.spinner_opt("thinking_color", _ACCENT)
_THINK_INTERVAL = float(T.spinner_opt("thinking_interval_ms", 120))

# Rich only animates spinners it knows by name, so register this theme's
# faces once under a private name.
SPINNERS["swarn.lain"] = {"interval": _THINK_INTERVAL, "frames": _THINK_FRAMES}

# How long one verb stays on screen before the next is drawn.
_THINK_VERB_SECONDS = float(T.spinner_opt("thinking_verb_seconds", 1.5))
...
class _RotatingStatus:
    ...
    def __init__(self, verbs: list[str], period: float):
        self._verbs  = verbs
        self._period = period
        self._status = _console.status(self._next_text(), spinner="swarn.lain",
                                       spinner_style=_THINK_COLOR)
        self._stop   = threading.Event()
        self._thread = None

    def _next_text(self) -> Text:
        return Text(f"{_I}{random.choice(self._verbs)}… ", style=_THINK_COLOR)
...
def thinking_status(label: str | None = None):
    ...
    `thinking_verb_seconds`, so even one long LLM call cycles through
    phrases instead of freezing on one. No-op when stdout isn't a TTY."""
    if not _console.is_terminal:
        return nullcontext()
    if label:
        return _console.status(Text(f"{_I}{label}… ", style=_THINK_COLOR),
                               spinner="swarn.lain", spinner_style=_THINK_COLOR)
